...
import { pkgUrl1, pkgUrl2, pkgUrl3 } from '../constants/Urls';

export const pkgStockData1 = async () => {
  const result1 = await axios.get(pkgUrl1).then(res => {
  ...
  const result2 = await axios.get(pkgUrl3).then(res => {
    const $ = cheerio.load(res.data);
...
export const pkgStockData2 = async () => {
  const result = await axios.get(pkgUrl2).then(res => {
    const $ = cheerio.load(res.data);
    const openingPrice = $(
