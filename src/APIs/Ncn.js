...
import { ncnUrl1, ncnUrl2, ncnUrl3 } from '../constants/Urls';

export const ncnStockData1 = async () => {
  const result1 = await axios.get(ncnUrl1).then(res => {
  ...
  const result2 = await axios.get(ncnUrl3).then(res => {
    const $ = cheerio.load(res.data);
...
export const ncnStockData2 = async () => {
