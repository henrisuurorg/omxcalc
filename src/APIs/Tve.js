...
import { tveUrl1, tveUrl2, tveUrl3 } from '../constants/Urls';

export const tveStockData1 = async () => {
  const result1 = await axios.get(tveUrl1).then((res) => {
  ...
  const result2 = await axios.get(tveUrl3).then((res) => {
    const $ = cheerio.load(res.data);
...
export const tveStockData2 = async () => {
  const result = await axios.get(tveUrl2).then((res) => {
    const $ = cheerio.load(res.data);
    const openingPrice = $(
