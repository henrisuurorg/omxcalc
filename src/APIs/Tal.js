...
import { talUrl1, talUrl2, talUrl3 } from '../constants/Urls';

export const talStockData1 = async () => {
  const result1 = await axios.get(talUrl1).then(res => {
  ...
  const result2 = await axios.get(talUrl3).then(res => {
    const $ = cheerio.load(res.data);
...
export const talStockData2 = async () => {
  const result = await axios.get(talUrl2).then(res => {
    const $ = cheerio.load(res.data);
    const openingPrice = $(
