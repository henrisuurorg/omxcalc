...
import { sfgUrl1, sfgUrl2, sfgUrl3 } from '../constants/Urls';

export const sfgStockData1 = async () => {
  const result1 = await axios.get(sfgUrl1).then(res => {
  ...
  const result2 = await axios.get(sfgUrl3).then(res => {
    const $ = cheerio.load(res.data);
...
export const sfgStockData2 = async () => {
  const result = await axios.get(sfgUrl2).then(res => {
    const $ = cheerio.load(res.data);
    const openingPrice = $(
